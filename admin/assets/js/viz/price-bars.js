...
export function renderPriceBars({ canvas, competitors, ourPriceSgd = null }) {
  ...
  // Lemon Business palette: ink for competitor bars, lemon dashed line for "us" price anchor
  const datasets = [{ label: 'S$/mo', data, backgroundColor: 'rgba(33,29,18,0.78)' }];
  if (ourPriceSgd != null) {
    datasets.push({
      label: 'Us', data: labels.map(() => ourPriceSgd), type: 'line',
      borderColor: 'rgba(227,164,0,1)', borderWidth: 3, borderDash: [6, 4], pointRadius: 0, fill: false
    });
  }
